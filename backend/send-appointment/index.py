...
def handler(event: dict, context) -> dict:
    """Отправка заявки на запись в Telegram и на почту."""
    ...
    # --- Email via Mail.ru SMTP ---
    try:
        smtp_password = os.environ["SMTP_PASSWORD"]
        smtp_user = "comravet@mail.ru"

        msg = MIMEMultipart("alternative")
        msg["Subject"] = f"Новая запись: {name}"
        msg["From"] = smtp_user
        msg["To"] = smtp_user
        ...
        <div style="font-family: Arial, sans-serif; max-width: 480px; padding: 24px; border: 1px solid #e5e7eb; border-radius: 12px;">
          ...
          <table style="width: 100%; border-collapse: collapse;">
            ...
            <tr><td style="padding: 8px 0; color: #6b7280;">Причина</td><td style="padding: 8px 0;">{message or '—'}</td></tr>
          </table>
        </div>
        """
